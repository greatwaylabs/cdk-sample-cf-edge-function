...
test('given CF function event returns request', () => {
  const request = exampleEvent.request;
  const result = rewrite(exampleEvent);
  expect(result).toBe(request);
});

test('given match path request uri is rewritten', () => {
  // Deep clone object
  const event = JSON.parse(JSON.stringify(exampleEvent));
  event.request.uri = "/old_path/test.html";
  const result = rewrite(event);
  expect(result.uri).toBe("/new_path/test.html");
});
